...
@section('content')
    ...
    <div class="mt-3 rounded-2 overflow-hidden">
        <form action="{{route('admin.lesson.update',['id'=>$lesson->Id_Lesson])}}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="row">
                <div class="col-lg-6 mb-3">
                    <div class="bg-white p-3 rounded-3">
                        <div class="mb-3">
                            ...
                            <input type="text" name="Title" value="{{$lesson->Title}}" class="form-control" id="" placeholder="Tiêu đề bài">
                            @error('Title')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="Id_Chapter" aria-label="Default select example">
                                ...
                                @foreach ($chapter_list as $item)
                                    <option value="{{$item->Id_Chapter}}" {{($item->Id_Chapter==$lesson->Id_Chapter)?'selected':''}}>{{$item->Title}}</option>
                                @endforeach
                            </select>
                            @error('Id_Chapter')
                                <div class="text-danger">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="STT" value="{{$lesson->STT}}" class="form-control" id="" placeholder="STT">
                            @error('STT')
                                <div class="text-danger">
                                    {{$message}}
                                </div>
                ...
                <div class="col-lg-6 mb-3">
                    <div class="bg-white p-3 rounded-3">
                        <div class="mb-3">
                            ...
                            <input type="text" name="Video_Lesson" value="{{$lesson->Video_Lesson}}" class="form-control" id="">
                            @error('Video_Lesson')
                                <div class="text-danger">
                                    {{$message}}
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="Hide" aria-label="Default select example">
                                <option value="1" {{($lesson->Hide==1)?'selected':''}}>Hiện</option>
                                <option value="0" {{($lesson->Hide==0)?'selected':''}}>Ẩn</option>
                              </select>
                        </div>
                        <button type="submit" class="btn btn-primary">Sửa bài học</button>
                    </div>
                </div>
